...
// This copy is a static overhead diorama of the full gallery. It deliberately
// shares the gallery's geometry and artwork, but uses an isolated scaled root,
// a roofless view, orbit controls, and poster frames instead of videos.
const DIORAMA_MODE = true;
const DIORAMA_SCALE = 0.12;
const DIORAMA_CENTER_X = -5.5;
const DIORAMA_CENTER_Z = 29.3;
const DIORAMA_STAND_TOP = 0.95;
const EMPTY_DIORAMA_TABLE_X = 8.2;
...
const resumePages = [
  {
    image: './resume/resume-page-1.jpg?v=20260723-resume-update3',
    width: 3.28,
    aspect: 1391 / 1800,
  ...
  {
    image: './resume/resume-page-2.jpg?v=20260723-resume-update3',
    width: 3.28,
    aspect: 1391 / 1800,
    position: [-1.9, 2.72, 6.9],
    rotationY: Math.PI
...
let controls;
let dioramaWorld;
let emptyDioramaWorld;
let rawSceneAdd;
let dioramaRoofPassNeeded = false;
let dioramaExteriorCeilingsAdded = false;
const dioramaOrbit = {
...
const shrimpRoomMusic = new Audio(DIORAMA_MODE ? '' : './media/shrimp-game-music.mp3?v=20260717');
shrimpRoomMusic.loop = true;
shrimpRoomMusic.preload = DIORAMA_MODE ? 'none' : 'auto';
shrimpRoomMusic.volume = 0;
shrimpRoomMusic.setAttribute('aria-hidden', 'true');
const shrimpRoomWater = new Audio(DIORAMA_MODE ? '' : './media/shrimp-game-water.mp3?v=20260717');
shrimpRoomWater.loop = true;
shrimpRoomWater.preload = DIORAMA_MODE ? 'none' : 'auto';
...
function createAppRoom() {
  ...
  const hallWallHeight = DIORAMA_MODE ? ROOM.height : APP_ROOM.doorHeight;
  const hallWallGeometry = new THREE.BoxGeometry(ROOM_WALL_THICKNESS, hallWallHeight, wallLength);
  const leftHallWall = new THREE.Mesh(hallWallGeometry, wallMaterial);
  leftHallWall.position.set(doorLeft - ROOM_WALL_THICKNESS / 2, hallWallHeight / 2, wallStartZ + wallLength / 2);
...
function createGameRoomShell(room, doorSide) {
  ...
  const corridorWallHeight = DIORAMA_MODE ? ROOM.height : room.doorHeight;
  ...
  if (appCorridorLength > 0) {
    ...
    corridorSegments.forEach((segment) => {
      ...
      [doorNorthZ - ROOM_WALL_THICKNESS / 2, doorSouthZ + ROOM_WALL_THICKNESS / 2].forEach((z) => {
        const corridorWall = new THREE.Mesh(new THREE.BoxGeometry(segmentLength, corridorWallHeight, ROOM_WALL_THICKNESS), segment.wallMaterial);
        corridorWall.position.set(segmentCenterX, corridorWallHeight / 2, z);
  ...
  if (hasNorthDoor && !room.openNorth) {
    ...
    const corridorWall = new THREE.Mesh(new THREE.BoxGeometry(ROOM_WALL_THICKNESS, corridorWallHeight, brainCorridorLength), wallMaterial);
    corridorWall.position.set(x, corridorWallHeight / 2, brainCorridorCenterZ);
      scene.add(corridorWall);
    });
  }
...
function createStandingVideoDisplay(work, posterTexture) {
  ...
  if (!DIORAMA_MODE && (work.eagerLoad || videoPreloadStarted)) {
    video.preload = 'auto';
    video.src = entry.source;
    video.load();
  }
  scene.add(group);
  if (galleryActive && !DIORAMA_MODE) requestVideoSync();
  needsRender = true;
  return entry;
}
...
function startBrainWallLoads() {
  ...
  const textureLoader = new THREE.TextureLoader();
  const sheets = BRAIN_WALL_SHEETS.filter((sheet) => !DIORAMA_MODE || sheet.side !== 'floor');
  scheduleLowPriorityGallerySequence(
    sheets.map((sheet) => () => {
...
function startDioramaVideoRoomPosterLoads() {
  if (!DIORAMA_MODE || videoRoomDisplaysLoaded) return;
...
function createScreeningRoom() {
  ...
  [
    ...
    scene.add(hallCeiling);
    const hallWallHeight = DIORAMA_MODE ? ROOM.height : SCREENING_ROOM.doorHeight;
    const hallWallGeometry = new THREE.BoxGeometry(ROOM_WALL_THICKNESS, hallWallHeight, segmentWallLength);
    const leftHallWall = new THREE.Mesh(hallWallGeometry, segment.wallMaterial);
    leftHallWall.position.set(doorLeft - ROOM_WALL_THICKNESS / 2, hallWallHeight / 2, segment.wallStartZ + segmentWallLength / 2);
    scene.add(leftHallWall);
    const rightHallWall = leftHallWall.clone();
    rightHallWall.position.x = doorRight + ROOM_WALL_THICKNESS / 2;
    scene.add(rightHallWall);
...
function createBrainRoom() {
  ...
  const ceilingMaterial = new THREE.MeshStandardMaterial({ color: 0x000000, roughness: 1, metalness: 0 });
  const corridorWallHeight = DIORAMA_MODE ? ROOM.height : BRAIN_ROOM.doorHeight;
  ...
  [doorNorthZ - ROOM_WALL_THICKNESS / 2, doorSouthZ + ROOM_WALL_THICKNESS / 2].forEach((z) => {
    const corridorWall = new THREE.Mesh(
      new THREE.BoxGeometry(corridorLength, corridorWallHeight, ROOM_WALL_THICKNESS),
      wallMaterial
    );
    corridorWall.position.set(corridorCenterX, corridorWallHeight / 2, z);
    scene.add(corridorWall);
  });
...
function addVideoWork(work, posterTexture) {
  ...
  if (!DIORAMA_MODE && (work.eagerLoad || (videoPreloadStarted && !work.deferVideoLoad))) {
    video.preload = 'auto';
    video.src = entry.source;
    video.load();
  }
...
function applyDioramaCamera() {
  if (!camera) return;
  const horizontal = Math.cos(dioramaOrbit.elevation) * dioramaOrbit.distance;
  camera.position.set(
    dioramaOrbit.target.x + Math.sin(dioramaOrbit.azimuth) * horizontal,
    dioramaOrbit.target.y + Math.sin(dioramaOrbit.elevation) * dioramaOrbit.distance,
    dioramaOrbit.target.z + Math.cos(dioramaOrbit.azimuth) * horizontal
  );
  camera.lookAt(dioramaOrbit.target);
  camera.updateMatrixWorld();
}

function isDioramaFloorPlanPosition(x, z) {
  const insideRoom = (room) => {
    const nearZ = room === ROOM ? -ROOM.halfDepth : room.nearZ;
    const farZ = room === ROOM ? ROOM.halfDepth : room.farZ;
    const centerX = room === ROOM ? 0 : room.centerX;
    return x >= centerX - room.halfWidth
      && x <= centerX + room.halfWidth
      && z >= nearZ
      && z <= farZ;
  };
  if ([ROOM, APP_ROOM, SCREENING_ROOM, BRAIN_ROOM, GAME_ROOM, VIDEOS_ROOM, EMPTY_GAME_ROOM, ART_ROOM].some(insideRoom)) return true;

  const insideCorridor = (centerX, width, nearZ, farZ) => (
    x >= centerX - width / 2
    && x <= centerX + width / 2
    && z >= Math.min(nearZ, farZ)
    && z <= Math.max(nearZ, farZ)
  );
  if (insideCorridor(APP_ROOM.centerX, APP_ROOM.doorWidth, ROOM.halfDepth, APP_ROOM.nearZ)) return true;
  if (insideCorridor(SCREENING_ROOM.centerX, SCREENING_ROOM.doorWidth, ROOM.halfDepth, SCREENING_ROOM.nearZ)) return true;
  if (insideCorridor(ART_ROOM.centerX + ART_ROOM.doorOffsetX, ART_ROOM.doorWidth, APP_ROOM.farZ, ART_ROOM.nearZ)) return true;
  if (
    x >= SCREENING_ROOM.centerX + SCREENING_ROOM.halfWidth
    && x <= BRAIN_ROOM.centerX - BRAIN_ROOM.halfWidth
    && z >= BRAIN_HALL_NORTH_EDGE
    && z <= BRAIN_HALL_SOUTH_EDGE
  ) return true;
  return false;
}

function createDioramaExteriorCeilings() {
  if (!dioramaWorld || dioramaExteriorCeilingsAdded) return;
  dioramaExteriorCeilingsAdded = true;

  // Cap only the gaps outside the gallery floor plan. Room and hallway
  // interiors remain open for the overhead cutaway view.
  const minX = -25.6;
  const maxX = 18.2;
  const minZ = -7.4;
  const maxZ = 65.9;
  const tileSize = 2.4;
  const ceilingMaterial = new THREE.MeshBasicMaterial({ color: 0xf1f0ec });
  for (let x = minX; x < maxX; x += tileSize) {
    for (let z = minZ; z < maxZ; z += tileSize) {
      const width = Math.min(tileSize, maxX - x);
      const depth = Math.min(tileSize, maxZ - z);
      const centerX = x + width / 2;
      const centerZ = z + depth / 2;
      if (isDioramaFloorPlanPosition(centerX, centerZ)) continue;
      const ceiling = new THREE.Mesh(
        new THREE.BoxGeometry(width, 0.08, depth),
        ceilingMaterial
      );
      ceiling.position.set(centerX, ROOM.height + 0.08, centerZ);
      ceiling.userData.dioramaExteriorCeiling = true;
      dioramaWorld.add(ceiling);
    }
  }
}

function addDioramaStandAt(x, z) {
  const standBody = new THREE.Mesh(
    new THREE.BoxGeometry(6.55, 0.78, 10.6),
    new THREE.MeshBasicMaterial({ color: 0x777268 })
  );
  standBody.position.set(x, 0.39, z);
  rawSceneAdd(standBody);

  const standTop = new THREE.Mesh(
    new THREE.BoxGeometry(6.82, 0.14, 10.87),
    new THREE.MeshBasicMaterial({ color: 0xc8bba2 })
  );
  standTop.position.set(x, 0.85, z);
  rawSceneAdd(standTop);

  const edge = new THREE.Mesh(
    new THREE.BoxGeometry(6.9, 0.04, 10.95),
    new THREE.MeshBasicMaterial({ color: 0xc8bba2 })
  );
  // Keep the table surface just below the gallery floors so room floors are
  // visible instead of being covered by the pedestal plate.
  edge.position.set(x, 0.92, z);
  rawSceneAdd(edge);
}

function addEmptyRoomShell(world, room, floorColor) {
  ...
  const floor = new THREE.Mesh(
    new THREE.BoxGeometry(room.halfWidth * 2, 0.1, roomDepth),
    floorMaterial
  );
...
function addEmptyCorridor(world, centerX, centerZ, width, depth, floorColor, wallColor = 0xdeddd8) {
  const floor = new THREE.Mesh(
    new THREE.BoxGeometry(width, 0.08, depth),
  ...
  const wallMaterial = new THREE.MeshBasicMaterial({ color: wallColor });
  const wallGeometry = new THREE.BoxGeometry(ROOM_WALL_THICKNESS, ROOM.height, depth);
  const leftWall = new THREE.Mesh(wallGeometry, wallMaterial);
  leftWall.position.set(centerX - width / 2 - ROOM_WALL_THICKNESS / 2, ROOM.height / 2, centerZ);
  world.add(leftWall);
...
function createEmptyDioramaModel() {
  emptyDioramaWorld = new THREE.Group();
  emptyDioramaWorld.name = 'Empty miniature gallery model';
  emptyDioramaWorld.scale.setScalar(DIORAMA_SCALE);
  emptyDioramaWorld.position.set(
    EMPTY_DIORAMA_TABLE_X - DIORAMA_CENTER_X * DIORAMA_SCALE,
    DIORAMA_STAND_TOP,
    -DIORAMA_CENTER_Z * DIORAMA_SCALE
  );
  rawSceneAdd(emptyDioramaWorld);

  addEmptyRoomShell(emptyDioramaWorld, ROOM, 0xd3d2cd);
  addEmptyRoomShell(emptyDioramaWorld, APP_ROOM, 0xbfc1bf);
  addEmptyRoomShell(emptyDioramaWorld, SCREENING_ROOM, 0x0b0c0d);
  addEmptyRoomShell(emptyDioramaWorld, BRAIN_ROOM, 0x000000);
  addEmptyRoomShell(emptyDioramaWorld, GAME_ROOM, 0x020202);
  addEmptyRoomShell(emptyDioramaWorld, VIDEOS_ROOM, 0x020202);
  addEmptyRoomShell(emptyDioramaWorld, EMPTY_GAME_ROOM, 0x020202);
  addEmptyRoomShell(emptyDioramaWorld, ART_ROOM, 0xaaa397);

  addEmptyCorridor(
    emptyDioramaWorld,
    APP_ROOM.centerX,
    (ROOM.halfDepth + APP_ROOM.nearZ) / 2,
    APP_ROOM.doorWidth,
    APP_ROOM.nearZ - ROOM.halfDepth,
    0xbfc1bf
  );
  addEmptyCorridor(
    emptyDioramaWorld,
    SCREENING_ROOM.centerX,
    (ROOM.halfDepth + SCREENING_ROOM.nearZ) / 2,
    SCREENING_ROOM.doorWidth,
    SCREENING_ROOM.nearZ - ROOM.halfDepth,
    0xd3d2cd
  );
  addEmptyCorridor(
    emptyDioramaWorld,
...
function createDioramaEnvironment() {
  if (!rawSceneAdd) return;

  // Match the original gallery's neutral floor exactly.
  const floorMaterial = new THREE.MeshBasicMaterial({ color: 0xd3d2cd });
  const floor = new THREE.Mesh(new THREE.PlaneGeometry(72, 72), floorMaterial);
  floor.rotation.x = -Math.PI / 2;
  floor.position.y = -0.02;
  floor.receiveShadow = true;
  rawSceneAdd(floor);

  const wallMaterial = new THREE.MeshBasicMaterial({ color: 0xe9e7e1 });
  const backWall = new THREE.Mesh(new THREE.BoxGeometry(72, 18, 0.3), wallMaterial);
  backWall.position.set(0, 9, -22);
  rawSceneAdd(backWall);
  const sideWall = new THREE.Mesh(new THREE.BoxGeometry(0.3, 18, 72), wallMaterial);
  sideWall.position.set(-28, 9, 0);
  rawSceneAdd(sideWall);

  addDioramaStandAt(0, 0);

  rawSceneAdd(new THREE.HemisphereLight(0xfffcf5, 0x8b9192, 1.25));
  const keyLight = new THREE.DirectionalLight(0xfff8ed, 1.25);
  keyLight.position.set(8, 18, 10);
  rawSceneAdd(keyLight);
  const fillLight = new THREE.DirectionalLight(0xdcecff, 0.45);
  fillLight.position.set(-12, 9, -8);
  rawSceneAdd(fillLight);
}

function setupDioramaWorld() {
  rawSceneAdd = scene.add.bind(scene);
  dioramaWorld = new THREE.Group();
  dioramaWorld.name = 'Miniature gallery model';
  dioramaWorld.scale.setScalar(DIORAMA_SCALE);
  dioramaWorld.position.set(
    -DIORAMA_CENTER_X * DIORAMA_SCALE,
    DIORAMA_STAND_TOP,
    -DIORAMA_CENTER_Z * DIORAMA_SCALE
  );
  rawSceneAdd(dioramaWorld);

  // Route all gallery geometry into the scaled model while keeping the room,
  // pedestal, and lighting at normal scene scale.
  scene.add = (...objects) => {
    objects.filter(Boolean).forEach((object) => dioramaWorld.add(object));
    dioramaRoofPassNeeded = true;
    return scene;
  };
  createDioramaEnvironment();
}

function applyDioramaRoofPass() {
  if (!dioramaWorld || !dioramaRoofPassNeeded) return;
  dioramaWorld.updateMatrixWorld(true);
  const worldPosition = new THREE.Vector3();

  dioramaWorld.traverse((object) => {
    if (object.isLight) {
      object.visible = false;
      return;
    }
    if (object.userData.dioramaExteriorCeiling) return;
    if (!object.isMesh || !object.geometry) return;
    object.getWorldPosition(worldPosition);
    dioramaWorld.worldToLocal(worldPosition);
    ...
    const sourceColor = object.material?.color?.getHex?.();
    const isOriginalFloorColor = [0xd3d2cd, 0xbfc1bf, 0xaaa397].includes(sourceColor);
    if (
      (worldPosition.y < 0.2 || isOriginalFloorColor)
      && localSize
      && Math.max(localSize.x, localSize.z) > 4
    ) {
      // Preserve each room's original gallery floor color exactly. The
      // miniature uses unlit materials so these values remain stable without
      // recreating the original room light hotspots.
      object.material = new THREE.MeshBasicMaterial({ color: sourceColor ?? 0xd3d2cd });
    }
    // Remove every elevated horizontal slab, including hallway ceilings.
    if (
      localSize
      && localSize.y < 0.2
      && Math.max(localSize.x, localSize.z) > 1.5
      && worldPosition.y > 1.5
    ) {
      object.visible = false;
    }
    // Remove ceilings and ceiling light strips so the rooms read as a cutaway
    // architectural model from the overhead camera.
    if (worldPosition.y + localMinY > 5.15) object.visible = false;
  });
  dioramaRoofPassNeeded = false;
}

function startDiorama() {
  galleryActive = true;
  enteredOnce = true;
  dragLookEnabled = true;
  galleryApp.classList.add('drag-look');
  walkHint.textContent = 'Drag to orbit   Scroll to zoom   Static poster frames only';
  loadingScreen.hidden = true;
  welcomeScreen.hidden = true;
  applyDioramaCamera();

  // Load all rooms as static artwork. Video elements are created only as
  // inactive placeholders; their sources are never attached in diorama mode.
  startMainRoomBackgroundLoads();
  startResearchRoomLoads();
  startBrainWallLoads();
  requestBrainSurfaceLoad();
  startMriWallLoads();
  startArtRoomLoads();
  startGameRoomLoads();
  startDioramaVideoRoomPosterLoads();
  needsRender = true;
}
...
function finishLoading() {
  ...
  loadingStatus.textContent = DIORAMA_MODE ? 'Miniature ready.' : 'Gallery ready.';
  warmInitialCamera();
  if (DIORAMA_MODE) {
    startDiorama();
    return;
...
function initializeGallery() {
  ...
  renderer.setClearColor(DIORAMA_MODE ? 0xe1dfda : 0xe5e4e0, 1);

  scene = new THREE.Scene();
  scene.background = new THREE.Color(DIORAMA_MODE ? 0xe1dfda : 0xe5e4e0);
  scene.fog = new THREE.Fog(DIORAMA_MODE ? 0xe1dfda : 0xe5e4e0, DIORAMA_MODE ? 24 : 22, 78);
  ...
  if (!DIORAMA_MODE) MRI_VIDEO_WORKS.forEach((work) => addMriVideoWork(work));
...
canvas.addEventListener('pointermove', (event) => {
  ...
  if (DIORAMA_MODE) {
    dioramaOrbit.azimuth -= movementX * 0.006;
    dioramaOrbit.elevation = THREE.MathUtils.clamp(
      dioramaOrbit.elevation + movementY * 0.005,
      0.32,
      1.28
    );
    applyDioramaCamera();
...
canvas.addEventListener('wheel', (event) => {
  if (!DIORAMA_MODE || !galleryActive) return;
  event.preventDefault();
  dioramaOrbit.distance = THREE.MathUtils.clamp(
    dioramaOrbit.distance * Math.exp(event.deltaY * 0.0012),
    7.2,
    22
  );
  applyDioramaCamera();
